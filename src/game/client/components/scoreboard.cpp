	...
	for(int RenderDead = 0; RenderDead < 2; RenderDead++)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			// name
			{
				...
				if(ShowRClientIndicator && Cursor.m_LineWidth > 0.0f)
				{
					...
					if(Cursor.m_LineWidth > IndicatorAdvance)
					{
						...
						const ColorRGBA IconColor = VoiceEnabled ?
							ColorRGBA(0.0f, 165.0f / 255.0f, 250.0f / 255.0f, TextColor.a) :
							ColorRGBA(1.0f, 0.0f, 0.0f, TextColor.a);
						Graphics()->TextureSet(g_pData->m_aImages[IMAGE_RIICON_WHITE].m_Id);
						Graphics()->QuadsBegin();
						Graphics()->SetColor(IconColor);
						Graphics()->SelectSprite(SPRITE_RI_ICON_WHITE);
				...
				TextRender()->TextEx(&Cursor, GameClient()->m_Chat.FilterText(ClientData.m_aName));
			...
			// clan
			{
				...
				CTextCursor Cursor;
				const char *FilteredClan = GameClient()->m_Chat.FilterText(ClientData.m_aClan);
				Cursor.SetPosition(vec2(ClanOffset + (ClanLength - minimum(TextRender()->TextWidth(FontSize, FilteredClan), ClanLength)) / 2.0f, Row.y + (Row.h - FontSize) / 2.0f));
				Cursor.m_FontSize = FontSize;
				Cursor.m_Flags |= TEXTFLAG_ELLIPSIS_AT_END;
				Cursor.m_LineWidth = ClanLength;
				TextRender()->TextEx(&Cursor, FilteredClan);
			}
...
CUi::EPopupMenuFunctionResult CScoreboard::CScoreboardPopupContext::Render(void *pContext, CUIRect View, bool Active)
{
	...
	if(!pPopupContext->m_IsLocal || g_Config.m_RiScoreboardAlwaysShowQuickActions)
	{
		...
		if(pUi->DoButton_FontIcon(&pPopupContext->m_TrackerAction, FontIcon::RC_LIST_TRACK, IsTracked, &Action, BUTTONFLAG_LEFT, ActionCorners, true, TrackerActionColor))
		{
			if(IsTracked)
			{
				pScoreboard->GameClient()->m_RClient.TargetPlayerPosRemove(Client.m_aName);
			}
			else
			{
				pScoreboard->GameClient()->m_RClient.TargetPlayerPosAdd(Client.m_aName);
			}
		}
	...
	{
		pScoreboard->GameClient()->m_RClient.CopySkin(Client.m_aName);
	}
	...
	{
		pScoreboard->GameClient()->m_RClient.FetchFindHours(Client.m_aName, "");
	}
